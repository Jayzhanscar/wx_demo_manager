	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bofuncGoodsListom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-button type="primary" v-on:click="getOrderGoodsList(1, filters.order_num)">查询</el-button>
				</el-form-item>
				<!--<el-form-item>-->
		...
		<!--列表-->
		<el-table :data="orderGoodsList" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="order_type" label="订单类型" width="120" :formatter="formatType" sortable>
			</el-table-column>
			<el-table-column prop="money" label='支付金额' width="120" sortable>
			</el-table-column>
			<el-table-column prop="user_name" label='收件人' width="100" sortable>
			</el-table-column>
			<el-table-column prop="phone" label='手机号' width="140" sortable>
			</el-table-column>
			<el-table-column prop="status" label='发货状态' width="120" sortable>
			</el-table-column>
			<el-table-column prop="succeed" label='是否支付' width="120" :formatter="formatPay" sortable>
			</el-table-column>
			<el-table-column prop="province" label='省' width="100" sortable>
			</el-table-column>
			<el-table-column prop="city" label='市' width="100" sortable>
			</el-table-column>
			<el-table-column prop="district" label='区/县' width="100" sortable>
			</el-table-column>
...
<script>
	...
	export default {
		...
		create:function(){
			this.getOrderGoodsList(1,0);
		},
		methods: {
			// 订单类型
			formatType: function (row, column) {
				return row.order_type == 1 ? '拼团商品' : row.order_type == 0 ? '普通商品' : '未知';
			},
			// 支付状态
			...
			getOrderGoodsList: function (page, order_num) {
				...
				const success = data => {
					...
					this.orderGoodsList = data['results']
